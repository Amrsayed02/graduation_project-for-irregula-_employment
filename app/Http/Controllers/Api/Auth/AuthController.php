...
class AuthController extends Controller
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        if (!Auth::attempt($credentials)) {
            throw new \Illuminate\Auth\AuthenticationException(__('Kimlik doğrulama başarısız oldu'));
        }
        $user = auth()->user();
        if ($user->status == '0') {
            throw new \Illuminate\Auth\AuthenticationException(__('Kullanıcı engellendi'));
        }

        $token = $this->createTokenForUser($user);
        $this->updateFcm($user, $request->fcm);
        return response()->json(['token' => $token, "user" => $user, 'message' => 'Successful', 'status_code' => 200], 200);
    }
    ...
    public function register(RegisterRequest $request)
    {
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                $user->image = 'imagesfp/setting/' . $image;
                $user->save();
            }
